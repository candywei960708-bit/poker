...
## 1. 組別資訊
* **組別號碼**：[請填寫組別，例如：第 X 組]
* **系級班級**：資訊工程科/系 一年級 [請填寫班級，例如：資工1B]

## 2. 組員資訊
* **[姓名 A]** - [負責項目簡述，例如：主要架構設計、SFML 介面整合]
* **[姓名 B]** - [負責項目簡述，例如：Evaluator 牌型演算法實作]
* **[姓名 C]** - [負責項目簡述，例如：NPC 下注邏輯設計與測試]
...
* **雙介面互動**：使用 SFML 呈現精美的虛擬牌桌、撲克牌圖形與目前籌碼狀態；同時透過終端機 (Console) 進行詳細的下注操作（跟注、加注、棄牌、All-in）。
* **完整的德州撲克規則**：實作 Pre-flop、Flop、Turn、River 與 Showdown 五個階段。
* **自動牌型判定**：自動結算所有玩家手牌與公牌的最佳組合，精準判定高牌至同花順等牌型並分配彩池（Pot）。
* **NPC 隨機行為**：NPC 具備基本的隨機決策邏輯（包含加注、跟注、棄牌與 All-in）。

## 5. 程式介紹

### 核心類別與方法說明
* **`Game` 類別**：遊戲主控制中心。
  * `startNewRound()`：初始化牌局、洗牌並扣除底注 (Ante)。
  * `bettingRound()`：處理玩家與 NPC 的下注邏輯、更新彩池。
  * `proceedToNextStage()`：控制遊戲推進至下一階段（發公牌或進入結算）。
...
## 6. 程式使用方式
1. 執行程式後，請先在**命令列 (Console)** 輸入你想對戰的 NPC 數量 (1-5)。
